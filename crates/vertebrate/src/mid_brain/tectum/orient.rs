...
impl ThigmotaxisTectum {
    ...
    fn update(
        ...
        if self.exclusive.update(self.left.id, tick) {
            self.left.update(lateral_line, tick);
        }

        if self.exclusive.update(self.right.id, tick) {
            self.right.update(lateral_line, tick);
        }
    }
}
